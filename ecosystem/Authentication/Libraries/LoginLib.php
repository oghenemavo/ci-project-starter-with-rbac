...
use Ecosystem\Authentication\Models\{UserLog, UserRememberToken};
...
class LoginLib 
{
    ...
    public function authenticate_user(array $user_data, bool $remember) {
        $fetched = $this->check_user($user_data['email'], $user_data['password']);
        [$user, $result] = $fetched; // destructing PHP v7.1

        if ($user) {
            $this->set_user_session($user);

            // set login log
            $this->set_user_log($user);
        ...
        if ($cookie !== null) {
            ...
            if ($user !== null) {
                ...
                if ($remember_time && empty($same_agent)) {
                    $rbac = Services::rbac();
                    $role_info = $rbac->set_user_role($user->id);
                    if ($role_info) {
                        $this->set_user_session($user);
    ...
    protected function check_user(string $email, string $password):array {
        ...
        $record = $this->verify_login($email, $password);
        if(!$record) {
            ...
            if (!$record->is_active) {
                ...
                $role_info = $rbac->set_user_role($record->id);
                if (!$role_info || empty($role_info)) {
    ...
    public function set_user_log($user)
    {
        ...
        $user_data = [
            ...
            'user_agent'  => \json_encode($agent_info),
        ];
        try {
            return $user_log->save($user_data);
        } catch (\ReflectionException $e) {
            //throw $th;
        }
        return false;
